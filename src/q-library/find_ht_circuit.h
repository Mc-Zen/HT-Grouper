...
namespace Q {
	...
	public:
		...
		template<template<class, class> class Iterable, class Allocator>
		void setOperators(const Iterable<Pauli, Allocator>& RS) {
			numOperatorsPerSet = RS.size();
			auto numQubits = RS[0].numQubits();
			updateSize(numQubits, RS.size());
